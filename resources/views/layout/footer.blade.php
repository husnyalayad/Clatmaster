<footer class="relative pt-40 text-white bg-black site-footer pb-11">
	<div class="absolute inset-0" style="background: radial-gradient(circle at 0% -50%, #BE3144, #F05941, transparent, transparent, transparent)">
	</div>
	<!--<div class="absolute inset-x-0 -top-px">-->
	<!--	<svg class="w-full fill-body-bg" preserveAspectRatio="none" width="1440" height="86" viewBox="0 0 1440 86" xmlns="http://www.w3.org/2000/svg">-->
	<!--		<path d="M0 86C90 51 200 16 300 20C400 24 500 61 600 48C700 35 800 11 900 18C1000 24 1100 74 1200 61C1300 48 1400 14 1440 86V0H0V86Z"/>-->
	<!--	</svg>-->
	<!--</div>-->
	<div class="absolute inset-x-0 -top-px">
	<svg class="w-full fill-body-bg wave" preserveAspectRatio="none" width="1440" height="86" viewBox="0 0 1440 86" xmlns="http://www.w3.org/2000/svg">
		<path d="M0 86C90 51 200 16 300 20C400 24 500 61 600 48C700 35 800 11 900 18C1000 24 1100 74 1200 61C1300 48 1400 14 1440 86V0H0V86Z"/>
	</svg>
</div>

<style>
@keyframes wave-animation {
  0% {
    d: path("M0 86C90 51 200 16 300 20C400 24 500 61 600 48C700 35 800 11 900 18C1000 24 1100 74 1200 61C1300 48 1400 14 1440 86V0H0V86Z");
  }
  50% {
    d: path("M0 86C90 45 200 12 300 18C400 24 500 55 600 42C700 29 800 7 900 15C1000 22 1100 65 1200 54C1300 43 1400 12 1440 86V0H0V86Z");
  }
  100% {
    d: path("M0 86C90 51 200 16 300 20C400 24 500 61 600 48C700 35 800 11 900 18C1000 24 1100 74 1200 61C1300 48 1400 14 1440 86V0H0V86Z");
  }
}

.wave path {
  animation: wave-animation 5s infinite ease-in-out;
}
</style>
	<div class="relative">
		<div class="container mb-28">
			<div class="w-1/2 mx-auto text-center max-lg:w-10/12 max-sm:w-full">
				<p class="text-xs font-semibold tracking-widest uppercase mb-9"><span class="inline-block px-3 py-1 !me-2 rounded-xl bg-[#262626]">{{__($setting->site_name)}}</span> {{__($fSetting->footer_text_small)}}</p>
				<p class="text-[100px] font-bold font-oneset leading-none tracking-tight mb-8 text-transparent bg-clip-text bg-gradient-to-br from-transparent -from-[5%] to-white to-50% max-sm:text-[18vw]">{{__($fSetting->footer_header)}}</p>
				<p class="text-[20px] font-oneset leading-[1.25em] opacity-50 font-normal mb-9 px-10">Experience the power of our AI generative content tool with a free trial. Get started today to explore all features and see how it can transform your content creation process!
</p>
				<x-button link="{{$fSetting->footer_button_url}}" label="{{__($fSetting->footer_button_text)}}" size="lg" iconPos="end" bg="bg-white bg-opacity-10 border-[2px] border-white border-opacity-0 hover:bg-white hover:bg-opacity-100" text="hover:!text-black">
					<x-slot name="icon">
						<svg class="ml-2" width="11" height="14" viewBox="0 0 47 62" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
							<path d="M27.95 0L0 38.213H18.633V61.141L46.583 22.928H27.95V0Z"/>
						</svg>
					</x-slot>
				</x-button>
			</div>
		</div>
		<hr class="border-white border-opacity-15">
		<div class="container">
			<div class="flex flex-wrap items-center justify-between gap-8 pt-10 pb-7 max-sm:justify-center">
				<a href="{{route('index')}}">
					@if(isset($setting->logo_2x_path))
						<img src="/{{$setting->logo_path}}" srcset="/{{$setting->logo_2x_path}} 2x" alt="{{$setting->site_name}} logo">
					@else
						<img src="/{{$setting->logo_path}}" alt="{{$setting->site_name}} logo">
					@endif
				</a>
				
				<ul class="flex flex-wrap items-center gap-7 text-[14px] max-sm:justify-center">
				    

      <li>
            <a
              href="https://www.crunchbase.com/organization/kinoapp"
              class="inline-flex items-center gap-2"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                id="Layer_1"
                data-name="Layer 1"
                viewBox="0 0 64 64"
                width="16"
                height="16"
                style="margin-top: -5px; fill: #ffffff"
              >
                <title>logo_crunchbase</title>
                <path
                  style="
                    clip-rule: evenodd;
                    fill: #ffffff;
                    fill-opacity: 1;
                    fill-rule: evenodd;
                    stroke-width: 0.10474632;
                    stroke-linejoin: round;
                    stroke-miterlimit: 1.41400003;
                  "
                  d="m 64.000001,8.0000025 c 0,-4.41736 -3.58264,-8 -8,-8 H 7.9999985 c -4.41736,0 -8,3.58264 -8,8 V 55.999997 c 0,4.41736 3.58264,8 8,8 H 56.000001 c 4.41736,0 8,-3.58264 8,-8 z"
                ></path>
                <g
                  transform="matrix(2.3849106,0,0,2.3849106,-9.5449499,-43.547318)"
                  style="fill: #000"
                >
                  <path
                    style="
                      fill: #000;
                      fill-opacity: 1;
                      stroke-width: 0.57408553;
                    "
                    d="m 13.939838,33.65836 a 2.9622812,2.9622812 0 1 1 0.03444,-2.439863 h 2.296342 a 5.1667696,5.1667696 0 1 0 0,2.439863 h -2.296342 z"
                  ></path>
                  <path
                    style="
                      fill: #000;
                      fill-opacity: 1;
                      stroke-width: 0.57408553;
                    "
                    d="m 23.510509,27.257306 h -0.378897 a 5.0978793,5.0978793 0 0 0 -2.525976,0.889833 v -5.752337 h -2.095412 v 14.794184 h 2.106894 v -0.539641 a 5.1667696,5.1667696 0 1 0 2.893391,-9.392039 z m 2.962281,5.534185 v 0.09185 a 2.9393178,2.9393178 0 0 1 -0.08037,0.361674 v 0 a 2.933577,2.933577 0 0 1 -0.143521,0.373156 v 0.04593 a 2.9795038,2.9795038 0 0 1 -2.072449,1.624662 v 0 l -0.281302,0.04593 h -0.06315 a 2.9163544,2.9163544 0 0 1 -0.321488,0 v 0 a 2.9622812,2.9622812 0 0 1 -0.40186,-0.02871 H 23.0168 a 2.933577,2.933577 0 0 1 -0.752052,-0.229634 h -0.05741 a 2.9737629,2.9737629 0 0 1 -0.66594,-0.447787 v 0 a 2.9909855,2.9909855 0 0 1 -0.522417,-0.625753 v 0 a 2.9622812,2.9622812 0 0 1 -0.189449,-0.367414 v 0 a 2.9450587,2.9450587 0 0 1 0.03445,-2.439864 v 0 a 2.9680221,2.9680221 0 0 1 2.376714,-1.68207 2.933577,2.933577 0 0 1 0.304265,0 v 0 a 2.9680221,2.9680221 0 0 1 2.927836,2.881909 v 0 a 2.9565404,2.9565404 0 0 1 0,0.396119 z"
                  ></path>
                </g></svg
              >
            </a>
          </li>
          <li>
            <a
              href="https://www.f6s.com/kinoapp"
              class="inline-flex items-center gap-2"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
                viewBox="0 0 64 64"
                version="1.1"
                style="margin-top: -5px"
              >
                <title>F6S-logo</title>
                <desc>Created with Sketch.</desc>
                <defs></defs>
                <g
                  style="
                    fill: none;
                    fill-rule: evenodd;
                    stroke: none;
                    stroke-width: 1;
                  "
                  transform="scale(0.08)"
                >
                  <g>
                    <path
                      d="M 400,0 C 179.086,0 0,179.086 0,400 0,620.914 179.086,800 400,800 620.914,800 800,620.914 800,400 800,179.086 620.914,0 400,0 Z"
                      style="fill: #ffffff"
                    ></path>
                    <polygon
                      points="156.901,180.139 289.76,180.139 293.734,180.139 293.734,184.113 293.734,233.935 293.734,237.909 289.76,237.909 210.697,237.909 210.697,362.821 256.545,362.821 260.519,362.821 260.519,366.795 260.519,416.617 260.519,420.591 256.545,420.591 210.697,420.591 210.697,615.905 210.697,619.879 206.723,619.879 156.901,619.879 152.927,619.879 152.927,615.905 152.927,184.113 152.927,180.139 "
                      style="fill: #000"
                    ></polygon>
                    <path
                      d="m 372.428,237.909 v 124.912 h 68.517 c 16.582,0 31.127,14.476 31.127,30.977 v 194.721 c 0,16.705 -14.545,31.359 -31.127,31.359 h -95.027 c -16.59,0 -31.143,-14.289 -31.143,-30.578 V 212.296 c 0,-16.829 14.448,-32.156 30.313,-32.156 h 95.858 c 16.582,0 31.127,14.468 31.127,30.961 v 72.657 3.974 h -3.974 -49.822 -3.974 v -3.974 -45.849 z m 0.016,182.68 v 141.519 h 41.875 V 420.589 Z"
                      style="fill: #000"
                    ></path>
                    <path
                      d="m 647.073,283.74 v 3.974 h -3.974 -49.822 -3.974 v -3.974 -45.849 h -41.875 v 124.913 l 69.116,0.016 c 16.441,0 30.529,19.672 30.529,35.777 v 189.705 c 0,16.812 -14.266,31.558 -30.529,31.558 h -92.918 c -16.254,0 -30.512,-14.367 -30.512,-30.744 v -106.004 -3.966 l 3.966,-0.007 46.368,-0.083 3.981,-0.007 v 3.981 79.063 h 41.875 V 420.574 l -65.706,0.016 c -16.239,0 -30.484,-14.553 -30.484,-31.143 V 211.282 c 0,-16.599 14.258,-31.16 30.512,-31.16 h 92.918 c 16.263,0 30.529,14.561 30.529,31.16 z"
                      style="fill: #000"
                    ></path>
                  </g>
                </g></svg
              >
            </a>
          </li>
          <li>
            <a
              href="https://www.linkedin.com/company/kino-app/"
              class="inline-flex items-center gap-2"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="14"
                height="14"
                fill="currentColor"
                class="bi bi-linkedin"
                viewBox="0 0 16 16"
              >
                <path
                  d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854zm4.943 12.248V6.169H2.542v7.225zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248S2.4 3.226 2.4 3.934c0 .694.521 1.248 1.327 1.248zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016l.016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225z"
                /></svg
              >
            </a>
          </li>

          @if($setting->frontend_footer_instagram != null)
          <li>
            <a
              href="{{$setting->frontend_footer_instagram}}"
              class="inline-flex items-center gap-2"
            >
              <svg
                width="14"
                height="14"
                viewBox="0 0 14 14"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M6.565 3.897C6.12304 3.89674 5.68536 3.98364 5.27703 4.15275C4.8687 4.32185 4.49773 4.56982 4.18535 4.88247C3.87298 5.19513 3.62533 5.56632 3.4566 5.9748C3.28786 6.38329 3.20134 6.82104 3.202 7.263C3.20134 7.70496 3.28786 8.14272 3.4566 8.5512C3.62533 8.95968 3.87298 9.33087 4.18535 9.64353C4.49773 9.95618 4.8687 10.2042 5.27703 10.3733C5.68536 10.5424 6.12304 10.6293 6.565 10.629C7.00747 10.6301 7.44579 10.5437 7.85481 10.375C8.26384 10.2062 8.63552 9.95839 8.94853 9.64566C9.26154 9.33292 9.50972 8.96146 9.67884 8.55259C9.84795 8.14371 9.93466 7.70547 9.934 7.263C9.93466 6.82053 9.84795 6.38229 9.67884 5.97342C9.50972 5.56454 9.26154 5.19308 8.94853 4.88035C8.63552 4.56762 8.26384 4.31977 7.85481 4.15102C7.44579 3.98227 7.00747 3.89595 6.565 3.897ZM6.565 9.452C5.98521 9.45042 5.42966 9.21918 5.01996 8.80892C4.61026 8.39867 4.37979 7.8428 4.379 7.263C4.37979 6.6833 4.61029 6.12753 5.02002 5.71743C5.42975 5.30733 5.9853 5.07632 6.565 5.075C7.14557 5.075 7.70241 5.30543 8.11321 5.71567C8.52402 6.12591 8.75521 6.68243 8.756 7.263C8.75521 7.84366 8.52405 8.40028 8.11327 8.81069C7.7025 9.22109 7.14566 9.45174 6.565 9.452ZM10.857 3.759C10.857 3.55081 10.7743 3.35114 10.6271 3.20392C10.4799 3.05671 10.2802 2.974 10.072 2.974C9.86381 2.974 9.66414 3.05671 9.51692 3.20392C9.36971 3.35114 9.287 3.55081 9.287 3.759C9.28674 3.86216 9.30686 3.96436 9.34622 4.05972C9.38558 4.15508 9.44339 4.24172 9.51633 4.31467C9.58928 4.38761 9.67592 4.44543 9.77128 4.48478C9.86664 4.52414 9.96884 4.54427 10.072 4.544C10.1752 4.54427 10.2774 4.52414 10.3727 4.48478C10.4681 4.44543 10.5547 4.38761 10.6277 4.31467C10.7006 4.24172 10.7584 4.15508 10.7978 4.05972C10.8371 3.96436 10.8573 3.86216 10.857 3.759ZM13.086 4.559C13.1076 3.53821 12.7264 2.54995 12.025 1.808C11.2821 1.10849 10.2952 0.727024 9.275 0.745002C8.191 0.683002 4.942 0.683002 3.858 0.745002C2.838 0.724773 1.85059 1.10452 1.107 1.803C0.407211 2.54558 0.02631 3.53283 0.0460018 4.553C-0.0149982 5.637 -0.0149982 8.891 0.0460018 9.97C0.024539 10.9908 0.405671 11.979 1.107 12.721C1.85075 13.4196 2.83779 13.8002 3.858 13.782C4.942 13.844 8.191 13.844 9.275 13.782C10.2958 13.8035 11.284 13.4223 12.026 12.721C12.7247 11.9774 13.1051 10.9902 13.086 9.97C13.148 8.891 13.148 5.64 13.086 4.556V4.559ZM11.686 11.136C11.5742 11.4179 11.4058 11.6738 11.1911 11.8879C10.9765 12.1021 10.7201 12.2699 10.438 12.381C9.16472 12.6426 7.862 12.7314 6.565 12.645C5.27 12.7295 3.96951 12.6407 2.698 12.381C2.41558 12.2696 2.15908 12.1013 1.9444 11.8866C1.72971 11.6719 1.56144 11.4154 1.45 11.133C1.1887 9.8607 1.0999 8.559 1.186 7.263C1.10109 5.96634 1.18987 4.66414 1.45 3.391C1.56144 3.10858 1.72971 2.85208 1.9444 2.63739C2.15908 2.42271 2.41558 2.25443 2.698 2.143C3.96942 1.88264 5.27009 1.79451 6.565 1.881C7.861 1.79626 9.16251 1.88505 10.435 2.145C10.7178 2.25587 10.9748 2.42368 11.19 2.63803C11.4052 2.85237 11.574 3.10867 11.686 3.391C11.9473 4.6633 12.0361 5.965 11.95 7.261C12.0364 8.55767 11.9476 9.86006 11.686 11.133V11.136Z"
                />
              </svg>
              {{__('Instagram')}}
            </a>
          </li>
          @endif @if($setting->frontend_footer_twitter != null)
          <li>
            <a
              href="{{$setting->frontend_footer_twitter}}"
              class="inline-flex items-center gap-2"
            >
              <svg
                width="16"
                height="13"
                viewBox="0 0 16 13"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M13.523 3.211C14.1228 2.77154 14.6441 2.23399 15.065 1.621C14.502 1.86701 13.9054 2.02745 13.295 2.097C13.9369 1.71577 14.4176 1.11344 14.647 0.403004C14.0446 0.761467 13.3844 1.01253 12.696 1.145C12.4083 0.837679 12.0604 0.59287 11.674 0.425818C11.2876 0.258766 10.871 0.173049 10.45 0.174004C10.0461 0.173741 9.64613 0.253099 9.27294 0.407539C8.89974 0.561979 8.56066 0.788472 8.27506 1.07406C7.98947 1.35966 7.76298 1.69875 7.60854 2.07194C7.4541 2.44514 7.37474 2.84511 7.375 3.249C7.37654 3.48431 7.402 3.71885 7.451 3.949C6.22796 3.88692 5.03162 3.56845 3.93958 3.01425C2.84754 2.46005 1.88419 1.68249 1.112 0.732003C0.834894 1.20462 0.690161 1.74314 0.693002 2.291C0.69329 2.79727 0.818693 3.29561 1.05806 3.74171C1.29742 4.18781 1.64332 4.56785 2.065 4.848C1.57743 4.82921 1.10119 4.69558 0.675003 4.458V4.491C0.673081 5.2019 0.917536 5.89151 1.36677 6.44248C1.81601 6.99345 2.44227 7.37175 3.139 7.513C2.8746 7.58015 2.6028 7.61374 2.33 7.613C2.13541 7.61162 1.94119 7.59557 1.749 7.565C1.94555 8.17652 2.32836 8.71128 2.84387 9.09446C3.35938 9.47764 3.98179 9.69006 4.624 9.702C3.53536 10.5562 2.19073 11.0187 0.807003 11.015C0.55914 11.0168 0.311384 11.0041 0.0650024 10.977C1.47289 11.8822 3.11226 12.3614 4.786 12.357C5.93682 12.365 7.07775 12.1442 8.14252 11.7075C9.20729 11.2708 10.1747 10.6269 10.9885 9.8132C11.8023 8.99948 12.4463 8.03216 12.8831 6.96744C13.3199 5.90272 13.5408 4.76182 13.533 3.611C13.533 3.477 13.533 3.344 13.523 3.211Z"
                />
              </svg>
             
            </a>
          </li>
          @endif @if($setting->frontend_footer_facebook != null)
          <li>
            <a
              href="{{$setting->frontend_footer_facebook}}"
              class="inline-flex items-center gap-2"
            >
              <svg
                width="15"
                height="15"
                viewBox="0 0 15 15"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M14.831 7.266C14.8313 6.31174 14.6435 5.36678 14.2784 4.48511C13.9134 3.60344 13.3782 2.80234 12.7034 2.12758C12.0287 1.45281 11.2276 0.917614 10.3459 0.552556C9.46421 0.187499 8.51925 -0.000262497 7.565 2.75432e-07C6.61074 -0.000262497 5.66578 0.187499 4.78411 0.552556C3.90244 0.917614 3.10133 1.45281 2.42657 2.12758C1.75181 2.80234 1.21661 3.60344 0.851552 4.48511C0.486495 5.36678 0.298733 6.31174 0.298996 7.266C0.299081 8.99629 0.916469 10.6698 2.04014 11.9856C3.16381 13.3013 4.72005 14.1731 6.429 14.444V9.366H4.584V7.266H6.43V5.666C6.39018 5.29215 6.43309 4.91411 6.5557 4.5587C6.67831 4.20329 6.87761 3.8792 7.13947 3.60942C7.40133 3.33964 7.71933 3.13078 8.07093 2.99764C8.42254 2.86449 8.79912 2.81033 9.174 2.839C9.71907 2.84661 10.2628 2.89407 10.801 2.981V4.768H9.884C9.72795 4.74729 9.56924 4.76193 9.41961 4.81084C9.26998 4.85976 9.13327 4.94169 9.01958 5.05057C8.90589 5.15946 8.81814 5.29251 8.76283 5.43989C8.70751 5.58727 8.68603 5.7452 8.7 5.902V7.266H10.715L10.393 9.366H8.7V14.444C10.4091 14.1733 11.9656 13.3017 13.0895 11.9859C14.2133 10.6701 14.8309 8.99644 14.831 7.266Z"
                />
              </svg>
              
            </a>
          </li>
          @endif

				</ul>
			</div>
			<ul class="flex flex-wrap items-center gap-2 text-[14px] max-sm:justify-center">
					<li>
						<a href="mailto:admin@clatmaster.com" class="inline-flex items-center gap-2">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#f05941" class="w-6 h-6">
							 <path stroke-linecap="round" stroke-linejoin="round" d="M3 8l9 6 9-6M3 8v8.25A2.25 2.25 0 005.25 18.5h13.5A2.25 2.25 0 0021 16.25V8M3 8l9 6 9-6"></path>
							</svg>
							<p class="text-[14px] opacity-60 !text-end">Email: admin@clatmaster.com</p>
						</a>
					</li>
					<li>
						<a href="mailto:admin@clatmaster.com" class="inline-flex items-center gap-2">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#f05941" class="w-6 h-6">
								<path stroke-linecap="round" stroke-linejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z"></path>
							</svg>
							<p class="text-[14px] opacity-60 !text-end">Phone: 070 5307754</p>
						</a>
					</li>
					
					
					<li>
						<a href="#" class="inline-flex items-center gap-2">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#f05941" class="w-6 h-6">
								<path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"></path>
								<path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"></path>
							</svg>
							<p class="text-[14px] opacity-60 !text-end">No 113 Kumaran Ratnam Rd, Colombo 00200 Sri Lanka.
							   </p>
						</a>
					</li>
					<li>
						<a href="mailto:admin@clatmaster.com" class="inline-flex items-center gap-2">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#f05941" class="w-6 h-6">
								 <path stroke-linecap="round" stroke-linejoin="round" d="M10 5v14M14 5v14"></path>
							</svg>
							<p class="text-[14px] opacity-60 !text-end">Privacy Policy</p>
						</a>
					</li>
					
					<li>
						<a href="mailto:admin@clatmaster.com" class="inline-flex items-center gap-2">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#f05941" class="w-6 h-6">
								<path stroke-linecap="round" stroke-linejoin="round" d="M10 5v14M14 5v14"></path>
							</svg>
							<p class="text-[14px] opacity-60 !text-end">Terms and Conditions</p>
						</a>
					</li>
					
				</ul>
			<hr class="border-white border-opacity-15">
			<div class="flex flex-wrap items-center justify-center gap-4 py-4 max-sm:text-center">
				<p class="text-[14px] opacity-60 !text-end">© 2024 <a href="https://clatmaster.com/"><span style="color:#f05941;">ClatMaster.com</span></a> All Rights Reserved.
				</p>
			</div>
		</div>
	</div>
</footer>
