...
<!--</div>-->
<!--@endif-->
<header class="site-header absolute inset-x-0  @if(!$fSectSettings->preheader_active) top-12 @else top-1 @endif z-50 text-white transition-[background,shadow] group/header [&.lqd-is-sticky]:bg-white [&.lqd-is-sticky]:shadow-[0_4px_20px_rgba(0,0,0,0.03)] [&.lqd-is-sticky]:text-black" style="">
	...
	<nav id="frontend-local-navbar" class="flex items-center justify-between py-4 border-b border-white px-7 border-opacity-10 relative text-[14px] opacity-0 max-sm:px-2 transition-all duration-500 group-[.page-loaded]/body:opacity-100 group-[.lqd-is-sticky]/header:border-black group-[.lqd-is-sticky]/header:border-opacity-5">
		...
		<div class="site-nav-container basis-1/3 transition-all max-lg:w-full max-lg:absolute max-lg:top-full max-lg:right-0 max-lg:bg-[#343C57] max-lg:text-white max-lg:overflow-hidden max-lg:max-h-0 [&.lqd-is-active]:max-lg:max-h-[calc(100vh-150px)]">
			<div class="max-lg:max-h-[inherit] max-lg:overflow-y-scroll">
				...
				<div class="block md:hidden relative group pt-6 pb-5 px-10 -mt-3 border-t border-white/5">
					...
					@foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
						@if(in_array( $localeCode, explode(',', $settings_two->languages) ))
							<a href="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}" rel="alternate" hreflang="{{ $localeCode }}" class="block py-3 border-b border-black border-opacity-5 last:border-none transition-colors hover:bg-black hover:bg-opacity-5">{{ country2flag(substr($properties['regional'], strrpos($properties['regional'], '_') + 1)) }} {{$properties['native']}}</a>
						@endif
					@endforeach
				</div>
				@endif
		...
		<div class="flex justify-end gap-2 basis-1/3 max-lg:basis-2/3">
			...
			@else
				...
				<x-button link="{{ LaravelLocalization::localizeUrl( route('register') ) }}" label="Be a Master" border="border-[2px] border-white !border-opacity-0" bg="bg-white !bg-opacity-10 hover:!bg-opacity-100 group-[.lqd-is-sticky]/header:bg-black group-[.lqd-is-sticky]/header:hover:!bg-opacity-100" text="text-white group-[.lqd-is-sticky]/header:text-black group-[.lqd-is-sticky]/header:hover:text-white" />
			@endauth
...
@if($fSetting->floating_button_active)
	<div id="myBtn" class="fixed bottom-8 end-8 z-50 hidden">
		<a data-fancybox="video-show" href="{{!empty($fSetting->floating_button_link) ? $fSetting->floating_button_link : '#'}}"  class="flex  gap-5 text-sm text-[#002A40] text-opacity-60 items-center bg-white py-3 px-3 rounded-xl shadow-lg transition-all duration-300 hover:shadow-md hover:scale-110 hover:-translate-y-1">
			...
			<p class="[&amp;_strong]:block">{!! __($fSetting->floating_button_small_text)!!} <strong class="text-black text-[0.9rem]">{!! __($fSetting->floating_button_bold_text) !!} &nbsp;</strong></p>
		</a>
	</div>
@endif