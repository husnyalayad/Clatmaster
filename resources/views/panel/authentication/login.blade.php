                    ...
                    <a href="{{ route('index') }}" class="navbar-brand">
                        @if (isset($setting->logo_dashboard))
                            <img src="/{{ $setting->logo_dashboard_path }}"
                                ...
                                alt="{{ $setting->site_name }}" class="group-[.navbar-shrinked]/body:hidden dark:hidden">
        ...
        <div class="container-fluid">
            <div class="row items-stretch min-h-[100vh] max-lg:pt-32 max-lg:pb-20">
                        ...
                        <div class="mx-auto col-lg-6">
                                ...
                                <div class="flex flex-wrap !gap-4 text-[13px] -tracking-[0.15px]]">
                                    @if ($setting->google_active)
                                        <a href="{{ route('login.google') }}"
                                            class="flex items-center w-[calc(50%-0.5rem)] z-10 grow !py-3 !px-4 border border-[--tblr-border-color] rounded-[10px] !no-underline text-heading transition-all hover:shadow-lg hover:-translate-y-1">
                                            <svg class="shrink-0 !me-2" width="22" height="22" viewBox="0 0 25 24"
                                    ...
                                    @if ($setting->github_active)
                                        <a href="{{ route('login.github') }}"
                                            class="flex items-center w-[calc(50%-0.5rem)] z-10 grow !py-3 !px-4 border border-[--tblr-border-color] rounded-[10px] !no-underline text-heading transition-all hover:shadow-lg hover:-translate-y-1">
                                            <svg class="shrink-0 !me-2" width="22" height="22" viewBox="0 0 24 24"
                                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    @if ($setting->twitter_active)
                                        <a href="{{ route('login.twitter') }}"
                                            class="flex items-center w-[calc(50%-0.5rem)] z-10 grow !py-3 !px-4 border border-[--tblr-border-color] rounded-[10px] !no-underline text-heading transition-all hover:shadow-lg hover:-translate-y-1">
                                            <svg class="shrink-0 !me-2" width="22" height="22" viewBox="0 0 24 20"
                                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path
                            ...
                            @if ($setting->privacy_enable == 1 && $setting->privacy_enable_login == 1)
                                <div class="mt-3 text-center text-muted">
                                    ...
                                    <a href="{{ url('/terms') }}" target="_blank" tabindex="-1"
                                        class="font-medium underline">{{ __('Terms and Conditions') }}</a>
                                    {{ __('and') }}
                                    <a href="{{ url('/privacy-policy') }}" target="_blank" tabindex="-1"
                                        class="font-medium underline">{{ __('Privacy Policy') }}</a>.
                                </div>
                            @endif
                            @if ($setting->register_active == 1)
                                <div class="mt-20 text-center text-muted">
                                    {{ __("Don't have account yet?") }} <a href="{{ route('register') }}" tabindex="-1"
                                        class="font-medium underline">{{ __('Sign up') }}</a>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="flex flex-col justify-center overflow-hidden bg-center bg-cover col-lg-6 max-lg:hidden"
                    style="background-image: url(/images/bg/bgauth.png)">
                    <img src="/images/bg/dash.png" alt="MagicAI Dashboard Mockup"
